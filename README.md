...
- "/report [닉네임]" 명령 사용해 같은 대화방에 있는 특정 사용자를 신고 할 수있다.
- 유저는 3번 신고 받으면 대화방에서 강제 퇴출 된다.
- 대화방을 나가면 신고 내역은 초기화 된다.

### 5. 전체 메세지 기능 - 구현 완료

- "/toALL [메시지]" 명령어 사용해 모든 유저들에게 메시지 전송 가능

### 6. 비밀방 기능 - 구현 완료

- "/create [비밀번호]" 명령어 사용해 비밀방 생성 가능
- 입장을 원하는 유저는 "/join [방번호] [비밀번호]" 명령을 사용해 번호가 맞으면 입장